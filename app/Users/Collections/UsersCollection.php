    ...
    public function transactionsByUser($transactions)
    {
        ...
        foreach ($transactions as $transaction) {
            ...
            if ($transaction->transactiontype == TransactionTypes::$credit) {
                $transaction->credit = number_format($transaction->amount, intval($currency['decimals']), $currency['decimalseparator'], $currency['thousandseparator']);
            } else {
                $transaction->credit = "-";
            }

            if ($transaction->transactiontype == TransactionTypes::$debit) {
                $transaction->debit =  number_format($transaction->amount, intval($currency['decimals']), $currency['decimalseparator'], $currency['thousandseparator']);
            } else {
                $transaction->debit = "-";
            }
