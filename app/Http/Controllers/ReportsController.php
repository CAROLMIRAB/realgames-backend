...
use App\Paginator\PaginatorDataTable;
...
class ReportsController extends Controller
{
     ...
     * @return \Illuminate\Http\JsonResponse
     */
    public function profitDataAdmin()
    {
        try {
            $client = \Request()->input('client');
            $dateFrom = \Request()->input('start-date');
            ...
            \Log::error('ReportsController.profitDataAdmin.catch', ['exception' => $ex]);
            $response = [
                'status' => 'FAILED',
                'title' => __('¡Error!'),
                'message' => __('Error al efectuar la operación.')
     ...
     * @return \Illuminate\Http\JsonResponse
     */
    public function getProfitGeneralClient(Request $request)
    {
        try {
            $startDate = $request->startDate;
            $endDate = $request->endDate;
            if (empty($startDate) && empty($endDate)) {
                ...
                $clients = $this->clientsRepo->getProfitClients(\Session::get('timezone'), $startDate, $endDate);
                $data = $this->reportsCollection->profitGeneral($clients);
                return Datatables::of($data)->make();
            }
        } catch (\Exception $ex) {
            \Log::error('ReportsController.getProfitGeneralClient.catch', ['exception' => $ex]);
            $response = [
                'status' => 'FAILED',
                'title' => __('¡Error!'),
                'message' => __('Error al efectuar la operación.')
     ...
     * @return \Illuminate\Http\JsonResponse
     */
    public function getProfitGeneralUser(Request $request)
    {
        try {
            $startDate = $request->startDate;
            $endDate = $request->endDate;
            ...
            if (empty($startDate) && empty($endDate)) {
                ...
                $clients = $this->clientsRepo->getProfitUsers(\Session::get('timezone'), $startDate, $endDate, $client);
                $data = $this->reportsCollection->profitGeneral($clients);
                return Datatables::of($data)->make();
            }
        } catch (\Exception $ex) {
     ...
     * @return \Illuminate\Http\JsonResponse
     */
    public function getTotalProfitUsers(Request $request)
    {
        try {
            $startDate = $request->startDate;
            $endDate = $request->endDate;
            $client = \Auth::user()->id;
            $totals = $this->clientsRepo->getProfitTotalUser(\Session::get('timezone'), $startDate, $endDate, $client);
            $response = $this->reportsCollection->dataResumeProfit($totals);
        } catch (\Exception $ex) {
            \Log::error('ReportsController.getProfitGeneralClient.catch', ['exception' => $ex]);
            $response = [
                'status' => 'FAILED',
     ...
     * @return \Illuminate\Http\JsonResponse
     */
    public function getTotalProfitClient(Request $request)
    {
        try {
            $startDate = $request->startDate;
            $endDate = $request->endDate;
            $totals = $this->clientsRepo->getProfitTotalClient(\Session::get('timezone'), $startDate, $endDate);
            $response = $this->reportsCollection->dataResumeProfit($totals);
        } catch (\Exception $ex) {
            \Log::error('ReportsController.getProfitGeneralClient.catch', ['exception' => $ex]);
            $response = [
    ...
    public function viewProfitAdmin()
    {
        ...
        return view('reports.profitbyclientadmin', $data);
    }
    ...
    /**
     * View profit general clients
     *
     * @return \Illuminate\Contracts\View\Factory|\Illuminate\View\View
     *
     */
    public function viewProfitGeneralClient()
    {
        return view('reports.profitgeneralclient');
    }

    /**
     * View profit general users
     *
     * @return \Illuminate\Contracts\View\Factory|\Illuminate\View\View
     *
     */
    public function viewProfitGeneralUser()
    {
        return view('reports.profitgeneraluser');
    }
