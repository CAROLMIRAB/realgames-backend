...
class CoreCollection
{
    ...
    public function language()
    {
        ...
        foreach ($languages as $key => $language) {
            ...
            $html .= sprintf(
                '<li class="%s li-language" data-lang="%s"><a href="%s">%s %s</a></li>',
                $active,
                $key,
    ...
    public function topPlayersData($players)
    {
        ...
        $div1 = "<div class='nav-tabs-custom'>
                    <ul class='nav nav-tabs'>
                        <li class='active'>
                        <a href='#tab_users' data-toggle='tab' class=''>" . __('Usuarios') . "</a>
                        </li>";
        ...
        foreach ($players as $item) {
            $tab = "tab_" . $i;
            $div1 .= " <li><a href='#" . $tab . "' data-toggle='tab' class='tabs' data-id='" . $item->id . "' data-tab='" . $tab . "'>" . $item->username . "</a></li>";
            ...
            $div3 .= "<div class='tab-pane' id='" . $tab . "' data-id=" . $item->id . "></div>";
            $i++;
        }
     ...
     * @return array
     */
    public function favoritesGamesByUserData($games, $spins)
    {
        $table = "<table class='table table-condensed'>
                ...
                <tr>
                  ...
                  <th>" . __('Juego') . "</th>
                  <th>" . __('Proveedor') . "</th>
                  <th style='width: 40px'>" . __('Frecuencia') . "</th>
                </tr>";
        foreach ($games as $item) {
            $percent = ($item->spins / $spins->spin) * 100;
            $img = "https://cdn.dotworkers.com/realgames/140x140/" . $item->image;

            $table .= "<tr>
                  <td><img src='" . $img . "' height='30' width='30'></td>
                  <td>" . $item->title . "</td>
                  <td>" . $item->software . "</td>
                  <td>
                    <div class='progress progress-xs'>
                      <div class='progress-bar progress-bar-primary' style='width: " . round($percent, 1) . "%'></div>
                    </div>
                  </td>
                  <td><span class='badge bg-light-blue'>" . round($percent, 1) . "%</span></td>
                </tr> ";
